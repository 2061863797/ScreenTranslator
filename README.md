...
| 方式 | 说明 |
|------|------|
| **① Release 压缩包（推荐）** | 打开本仓库 GitHub 页的 **Releases**，下载 `runtime-*.zip`（文件名以页面为准），**解压到项目根目录**，得到完整 `runtime\`（models / llama / paddlex） |
| ② 脚本 | 项目根：`.\scripts\download_runtime.ps1` 或 `.\setup.ps1 -DownloadRuntime` |
| ③ 官网手动 | 见 [runtime/README.md](./runtime/README.md) |

**解压要求：** zip 内为 `runtime/models`、`runtime/llama`、`runtime/paddlex`；解压后应有 `runtime\models\*.gguf`。

**不进 Git、也不用打进源码包的：** `venv/`、`config.json`、历史库、日志——每台机器 `setup.ps1` 生成。  
**大文件不进 Git 仓库**，只通过 **Release 附件**（或脚本/官网）分发。
...
- **没有 zip**：下载脚本或手动补，见 [runtime/README.md](./runtime/README.md)。
...
1. **推荐：** 打开仓库 **Releases** → 下载 **`runtime-*.zip`**  
   → 解压到**当前项目根**（应有 `.\runtime\models\`、`.\runtime\llama\`、`.\runtime\paddlex\`）  
2. 或：`.\scripts\download_runtime.ps1` / `.\setup.ps1 -DownloadRuntime`  
3. 或：按 [runtime/README.md](./runtime/README.md) 官网手动下  
...
| 类型 | 路径 |
|------|------|
| 程序 | `app/`、`run.py`、`launcher.py`、`scripts/`、`setup.ps1`、`build.ps1` |
| 模板与文档 | `config.example.json`、`README*.md`、`SETTINGS.en.md`、`LLAMA启动器说明.md`、`LICENSE`、`NOTICE` 等 |
| llama 入口脚本 | `启动llama.bat`（本体 exe 在 runtime zip 的 `llama\` 里） |

### 不进 Git

| 路径 | 原因 |
|------|------|
| `runtime/models/`、`runtime/llama/`、`runtime/paddlex/` | 大文件，**Release 压缩包**分发 |
...
```powershell
# 模型文件应约 1GB；没有该文件说明还没解 Release zip
...
| 现象 | 处理 |
|------|------|
| 模型缺失 / 无法加载 | 下 **Release 的 runtime zip** 解压到项目根 |
| 没有 runtime | Releases 附件，**不要**只下 Source code zip（源码包不含模型） |
...
| [LICENSE](./LICENSE) / [NOTICE](./NOTICE) | Apache-2.0 与第三方说明 |
